...
class TreePrinter {
    ...
    public static void show(No root) {
    ...
    private static void printNodeInternal(List<No> nodes, int level, int maxLevel) {
        ...
        List<No> newNodes = new ArrayList<>();
        for (No node : nodes) {
            if (node != null) {
                System.out.print(node.elemento);
                newNodes.add(node.filho_esquerda);
                newNodes.add(node.filho_direita);
            } else {
                newNodes.add(null);
                newNodes.add(null);
                System.out.print(" ");
        ...
        for (int i = 1; i <= endgeLines; i++) {
            for (int j = 0; j < nodes.size(); j++) {
                ...
                if (nodes.get(j).filho_esquerda != null)
                    System.out.print("/");
                else
                ...
                if (nodes.get(j).filho_direita != null)
                    System.out.print("\\");
                else
                    TreePrinter.printWhitespaces(1);
    ...
    private static <T extends Comparable<?>> int maxLevel(No node) {
        ...
        return Math.max(TreePrinter.maxLevel(node.filho_esquerda), TreePrinter.maxLevel(node.filho_direita)) + 1;
    }
